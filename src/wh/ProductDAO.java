...
//System.out.println(pstmt.toString());
	...
	public ArrayList<ProductAcc> getProductAccList(int productNo) throws Exception { 
		
		Connection con = dbconnect.getConnection(); 
		PreparedStatement pstmt = null; 
		ResultSet rs = null; 
		
		ArrayList<ProductAcc> alist = new ArrayList<ProductAcc>(); 
		
		try { 
			StringBuffer sqlBuf = new StringBuffer(); 

			sqlBuf.append("		select  a.productNo, ");
			sqlBuf.append("				a.accNo, ");
			sqlBuf.append("				a.accCnt ");
			sqlBuf.append("		  from productAcc a ");
			sqlBuf.append("		  left outer join accessory b ");
			sqlBuf.append("		    on a.accNo = b.accNo ");
			sqlBuf.append("		 where a.productNo = ? ");
			sqlBuf.append("		 order by b.accId;");
...
//System.out.println(pstmt.toString());
			...
			while(rs.next()) {
				...
				ProductAcc productAcc = new ProductAcc(rs.getInt(1), rs.getInt(2), rs.getInt(3));
				
				alist.add(productAcc); 
			} 
		}catch(Exception e) { 
			throw e;
		}finally { 
	...
	public void deleteProductAcc(int productNo) throws Exception  { 
		
		Connection con = dbconnect.getConnection(); 
		PreparedStatement pstmt = null; 
		
		try { 
			sql = "delete from productAcc where productNo = ?"; 
...
//System.out.println(pstmt.toString());
	...
	public void insertProductAcc(ProductAcc productAcc) throws Exception  { 
		Connection con = dbconnect.getConnection(); 
		PreparedStatement pstmt = null; 
		
		try { 
			sql = "insert into productAcc (productNo, accNo, accCnt, insertUserId, insertDatetime) VALUES (?,?,?,?, current_timestamp())"; 
			
			pstmt = con.prepareStatement(sql); 

			pstmt.setInt(1, productAcc.getProductNo()); 
			pstmt.setInt(2, productAcc.getAccNo()); 
			pstmt.setInt(3, productAcc.getAccCnt()); 
			pstmt.setString(4, productAcc.getInsertUserId());
